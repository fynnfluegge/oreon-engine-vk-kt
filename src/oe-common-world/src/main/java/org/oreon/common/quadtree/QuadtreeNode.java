...
public abstract class QuadtreeNode extends Renderable{
	...
	public void render()
	{
		...
		if (BaseContext.Companion.getConfig().getRenderReflection() || BaseContext.Companion.getConfig().getRenderRefraction()){
	...
	private void updateChildNodes(){
		...
		float distance = (BaseContext.Companion.getCamera().getPosition().sub(worldPos)).length();
