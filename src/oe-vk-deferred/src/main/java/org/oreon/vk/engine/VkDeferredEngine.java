...
public class VkDeferredEngine extends RenderEngine {
	...
	public void init() {
		...
		graphicsDevice = VkContext.INSTANCE.getDeviceManager().getDeviceBundle(DeviceType.MAJOR_GRAPHICS_DEVICE);
	    ...
	    VkContext.INSTANCE.getResources().setOffScreenFbo(offScreenFbo);
	    VkContext.INSTANCE.getResources().setTransparencyFbo(transparencyFbo);
	    ...
	    if (BaseContext.Companion.getConfig().getFxaaEnabled()){
	    ...
	    if (BaseContext.Companion.getConfig().getBloomEnabled()){
	    ...
	    swapChain = new SwapChain(graphicsDevice.getLogicalDevice(), graphicsDevice.getPhysicalDevice(),
	    		VkContext.INSTANCE.getSurface(), displayImageView.getHandle());
	...
	public void shutdown() {
		...
		swapChain.destroy();
		BaseContext.Companion.getCamera().shutdown();
		graphicsDevice.getLogicalDevice().destroy();
		VkContext.INSTANCE.getVkInstance().destroy();
	}
