...
public class Sun extends Renderable{
	...
	public Sun() {
		...
		getWorldTransform().setTranslation(BaseContext.Companion.getConfig().getSunPosition().normalize().mul(-2600));
	    ...
	    VkPipeline graphicsPipeline = new GraphicsPipeline(device.getHandle(),
				...
				VkContext.INSTANCE.getResources().getTransparencyFbo().getRenderPass().getHandle(),
				VkContext.INSTANCE.getResources().getTransparencyFbo().getColorAttachmentCount(),
				1,
	    ...
	    CommandBuffer mainCommandBuffer = new SecondaryDrawCmdBuffer(
	    		...
	    		VkContext.INSTANCE.getResources().getTransparencyFbo().getFrameBuffer().getHandle(),
	    		VkContext.INSTANCE.getResources().getTransparencyFbo().getRenderPass().getHandle(),
	    		0,
