...
class VkStaticTextPanel(text: String?, xPos: Int, yPos: Int, xScaling: Int, yScaling: Int,
    ...
    init {
        ...
        val vertexInput = VkVertexInput(VertexLayout.POS2D_UV)
        val vertexBuffer = BufferUtil.createByteBuffer(panel.vertices, VertexLayout.POS2D_UV)
        ...
        val graphicsPipeline: VkPipeline = GraphicsPipelineAlphaBlend(device.handle,
        ...
        val cmdBuffer: CommandBuffer = SecondaryDrawIndexedCmdBuffer(
                device.handle,
		...
		val mainRenderInfo = VkRenderInfo(commandBuffer = cmdBuffer, pipeline = graphicsPipeline,
            vertexInput = vertexInput, descriptorSets = descriptorSets, descriptorSetLayouts = descriptorSetLayouts)
