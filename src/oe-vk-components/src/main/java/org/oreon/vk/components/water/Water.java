...
public class Water extends Renderable{
	...
	public Water() {
		...
		VkDeviceBundle deviceBundle = VkContext.INSTANCE.getDeviceManager().getDeviceBundle(DeviceType.MAJOR_GRAPHICS_DEVICE);
	    ...
	    VkContext.INSTANCE.getResources().setReflectionFbo(reflectionFbo);
	    VkContext.INSTANCE.getResources().setRefractionFbo(refractionFbo);
		...
		imageView_dudv = new VkImageView(device.getHandle(),
				...
				VK_IMAGE_ASPECT_COLOR_BIT, Util.INSTANCE.getMipLevelCount(image_dudv.getMetaData()));
	    ...
	    dudvSampler = new VkSampler(device.getHandle(), VK_FILTER_LINEAR, false, 0,
	    		VK_SAMPLER_MIPMAP_MODE_LINEAR, Util.INSTANCE.getMipLevelCount(image_dudv.getMetaData()),
	    		VK_SAMPLER_ADDRESS_MODE_REPEAT);
	    normalSampler = new VkSampler(device.getHandle(), VK_FILTER_LINEAR, false, 0,
	    		VK_SAMPLER_MIPMAP_MODE_LINEAR, Util.INSTANCE.getLog2N(waterConfig.getN()),
	    		VK_SAMPLER_ADDRESS_MODE_REPEAT);
	    reflectionSampler = new VkSampler(device.getHandle(), VK_FILTER_LINEAR, false, 0,
	    		VK_SAMPLER_MIPMAP_MODE_LINEAR, Util.INSTANCE.getLog2N(reflectionFbo.getWidth()),
	    		VK_SAMPLER_ADDRESS_MODE_REPEAT);
	    refractionSampler = new VkSampler(device.getHandle(), VK_FILTER_LINEAR, false, 0,
	    		VK_SAMPLER_MIPMAP_MODE_LINEAR, Util.INSTANCE.getLog2N(refractionFbo.getWidth()),
	    		VK_SAMPLER_ADDRESS_MODE_REPEAT);
		...
		normalRenderer = new NormalRenderer(
				VkContext.INSTANCE.getDeviceManager().getDeviceBundle(DeviceType.MAJOR_GRAPHICS_DEVICE),
	...
	public void render(){
		...
		// render reflection
		BaseContext.Companion.getConfig().setClipplane(clipplane);
