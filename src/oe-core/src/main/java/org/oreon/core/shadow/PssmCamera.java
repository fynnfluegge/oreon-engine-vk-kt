...
public class PssmCamera {
	...
	private void updateFrustumCorners(){
		...
		Vec3f right = BaseContext.Companion.getCamera().getUp().cross(BaseContext.Companion.getCamera().getForward());
		
		float tanFOV = (float) Math.tan(Math.toRadians(BaseContext.Companion.getCamera().getFovY()/2));
		float aspectRatio = BaseContext.Companion.getCamera().getWidth()/BaseContext.Companion.getCamera().getHeight();
		...
		//center of planes
		Vec3f centerNear = BaseContext.Companion.getCamera().getPosition().add(BaseContext.Companion.getCamera().getForward().mul(zNear));
		Vec3f centerFar = BaseContext.Companion.getCamera().getPosition().add(BaseContext.Companion.getCamera().getForward().mul(zFar));
		
		Vec3f NearTopLeft = centerNear.add( (BaseContext.Companion.getCamera().getUp().mul(heightNear/2f)).sub((right.mul(widthNear/2f))) );
		Vec3f NearTopRight = centerNear.add( (BaseContext.Companion.getCamera().getUp().mul(heightNear/2f)).add((right.mul(widthNear/2f))) );
		Vec3f NearBottomLeft = centerNear.sub( (BaseContext.Companion.getCamera().getUp().mul(heightNear/2f)).sub((right.mul(widthNear/2f))) );
		Vec3f NearBottomRight = centerNear.sub( (BaseContext.Companion.getCamera().getUp().mul(heightNear/2f)).add((right.mul(widthNear/2f))) );
		
		Vec3f FarTopLeft = centerFar.add( (BaseContext.Companion.getCamera().getUp().mul(heightFar/2f)).sub((right.mul(widthFar/2f))) );
		Vec3f FarTopRight = centerFar.add( (BaseContext.Companion.getCamera().getUp().mul(heightFar/2f)).add((right.mul(widthFar/2f))) );
		Vec3f FarBottomLeft = centerFar.sub( (BaseContext.Companion.getCamera().getUp().mul(heightFar/2f)).sub((right.mul(widthFar/2f))) );
		Vec3f FarBottomRight = centerFar.sub( (BaseContext.Companion.getCamera().getUp().mul(heightFar/2f)).add((right.mul(widthFar/2f))) );
