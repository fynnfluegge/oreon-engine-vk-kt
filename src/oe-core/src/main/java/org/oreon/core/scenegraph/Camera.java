...
private final Vec3f yAxis = new Vec3f(0,1,0);
	...
	protected Camera(Vec3f position, Vec3f forward, Vec3f up)
	{
		...
		input = BaseContext.Companion.getInput();
	}
	...
	public void update(){
		...
		if(input.isButtonHolding(0) || input.isButtonHolding(2))
		{
			...
			if (dy != 0){
				...
				setRotYstride(Math.abs(getRotYamt() * CoreEngine.Companion.getCurrentFrameTime() * 10));
			}
			...
			if (dx != 0){
				...
				setRotXstride(Math.abs(getRotXamt() * CoreEngine.Companion.getCurrentFrameTime() * 10));
			}
			...
			glfwSetCursorPos(BaseContext.Companion.getWindow().getId(),
	...
	private void initfrustumPlanes()
	{
		...
		Vec4f leftPlane = new Vec4f(
				this.projectionMatrix.get(3, 0) + this.projectionMatrix.get(0, 0)
					...
					* ((double) BaseContext.Companion.getConfig().getFrameWidth()
					/ (double) BaseContext.Companion.getConfig().getFrameHeight()))),
				...
				this.frustumPlanes[0] = Util.INSTANCE.normalizePlane(leftPlane);
		...
		Vec4f rightPlane = new Vec4f(
				...
				this.frustumPlanes[1] = Util.INSTANCE.normalizePlane(rightPlane);
		...
		Vec4f botPlane = new Vec4f(
				...
				this.frustumPlanes[2] = Util.INSTANCE.normalizePlane(botPlane);
		...
		Vec4f topPlane = new Vec4f(
				...
				this.frustumPlanes[3] = Util.INSTANCE.normalizePlane(topPlane);
		...
		Vec4f nearPlane = new Vec4f(
				...
				this.frustumPlanes[4] = Util.INSTANCE.normalizePlane(nearPlane);
		...
		//far plane
				...
				this.frustumPlanes[5] = Util.INSTANCE.normalizePlane(farPlane);
	}
