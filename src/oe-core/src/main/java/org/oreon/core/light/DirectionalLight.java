...
public abstract class DirectionalLight extends Light{
	...
	protected DirectionalLight(){
		...
		this(BaseContext.Companion.getConfig().getSunPosition().normalize(),
			new Vec3f(BaseContext.Companion.getConfig().getAmbient()),
			BaseContext.Companion.getConfig().getSunColor(),
			BaseContext.Companion.getConfig().getSunIntensity());
	}
...
//			log.warn("DirectionalLight vector up " + up + " and direction " +  direction + " not orthogonal");
	...
	public void update(){
		...
		if (BaseContext.Companion.getCamera().isCameraRotated() ||
				BaseContext.Companion.getCamera().isCameraMoved()){
		...
		if (BaseContext.Companion.getInput().isKeyHolding(GLFW.GLFW_KEY_I)) {
		...
		if (BaseContext.Companion.getInput().isKeyHolding(GLFW.GLFW_KEY_K)) {
		...
		if (BaseContext.Companion.getInput().isKeyHolding(GLFW.GLFW_KEY_J)) {
		...
		if (BaseContext.Companion.getInput().isKeyHolding(GLFW.GLFW_KEY_L)) {
			setDirection(getDirection().add(new Vec3f(-0.00075f,0,0.00075f)).normalize());
			updateLightBuffer();
			updateShadowMatrices(true);
			updateLightUbo();
...
//			log.warn("DirectionalLight vector up " + up + " and direction " +  direction + " not orthogonal");
		...
		BaseContext.Companion.getConfig().setSunPosition(getDirection());
	}
